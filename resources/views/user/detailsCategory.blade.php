...
@section('content')
@include('user.audioplayer' , ['entry' => 'get-category-as-json' ,  'playlist_id' => $genreObj->idTheLoai])
...
<div class="content flex-1">
      ...
      <div class="flex text-alt2 absolute top-0 left-0 w-full h-[14rem] bg-black bg-opacity-50 p-[1.5rem]">
        <div>
          <div class="h-[2.5rem] flex breadcrumb">
            ...
            <a href="#" class="text-alt4 text-hover">{{$genreObj->TenTheLoai}}</a>
          </div>
          <div class="mt-[1.5rem] h-[1.5rem] flex items-center uppercase text-alt4 text-sm font-medium tracking-wide">Thể Loại</div>
          <div class="h-[4rem] flex items-center space-x-[0.5rem]">
            <h1 class="text-[3rem] text-white p-0 m-0 font-medium">{{$genreObj->TenTheLoai}}</h1>
    ...
    <div class="flex flex-col w-full flex-1 px-[1.25rem] py-[2rem] pr-0">
      <div class="flex space-x-[0.5rem]">
        ...
        <span class="text-xs text-alt mt-[0.6rem]">{{$genres->count()}} bài hát</span>
      </div>
          ...
          <tr class="currently-playing">
            ...
            <td class="w-[4rem]">
              <a href="#" class="flex ml-[0.5rem] w-max">
                <img src="{{$genre->HinhBaiHat}}" class="w-auto object-cover w-[2.5rem] h-[2.5rem] rounded-md" alt="">
              </a>
            </td>
            <td class="font-medium">
              <div class="flex-col !items-start">
                <a href="#" class="duration-300 flex w-max hover:translate-x-[0.25rem]">{{$genre->Casi}}</a>
            ...
            <td class="hidden md:table-cell">{{$genre->TenBaiHat}}</td>
            <td class="hidden sm:table-cell">
              <div>
                ...
                <span>{{$genre->LuotThich}}</span>
              </div>
            </td>

            <td class="hidden sm:table-cell"
            onclick="javascript:(function() { 
              window.location.href = '{{route('add-like' , ['id' => $genre->idBaiHat])}}';
            })()"
            >
